(function () {
    ...
    configNavBar();
    configCodeBox();
    CodeParse( getCodeToParse() );
    if ( isAutoHotkeyPage() ) onClickRunCodeButton();
    else $('.runCodeButton').each(function() { $(this).hide() });
    ...
    function configPage() {
        ...
        $("textarea#fld4").css({ fontFamily: "'Droid Sans Mono', monospace",
                                 height:     isMobile ? "300px" : "640px" });  /* textarea на странице «Персональное» */
    }

    function configNavBar() {
        var $nav = $('#navextra2'), ahkHref = 'http://forum.script-coding.com/viewforum.php?id=13', currentLiClass;
        if (!isMobile) {
    ...
    function isAutoHotkeyPage()  {
        if (isMobile) return false;
        var found;
        $('.crumb').each( function () {
            if ($(this).html().indexOf('AutoHotkey') != -1) return !(found = true);
        });
        return found;
    }

    function onClickRunCodeButton() {
        $(".runCodeButton").mousedown(function()  {
...
function getCodeToParse() {
    ...
    <img class="runCodeButton" style="margin-top:4px; cursor:pointer" src="data:image/png;base64,
